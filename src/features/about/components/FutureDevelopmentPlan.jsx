  ...
  return (
    <article className="h-full rounded-2xl border border-slate-200/80  bg-white/70  backdrop-blur-sm overflow-hidden">
      <div className="px-5 pt-5 pb-4 border-b border-slate-100  flex items-center gap-3">
        ...
        <div>
          <h3 className="text-sm font-bold text-slate-900 ">ভবিষ্যৎ উন্নয়ন পরিকল্পনা</h3>
          <p className="text-xs text-slate-400 ">অবকাঠামো, সুযোগ–সুবিধা ও সার্বিক উন্নয়নের রূপরেখা</p>
        </div>
      </div>
      <ul className="p-5 space-y-3">
        {aboutData.futurePlans.development.map((plan, index) => (
          <li key={index} className="flex items-start gap-3 text-sm">
            ...
            <span className="text-slate-600  leading-relaxed text-justify">{plan}</span>
          </li>
        ))}
      </ul>
    </article>
